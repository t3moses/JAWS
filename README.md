...
The JAWS frontend application is located in the `public/app/` directory. This provides the user interface for interacting with the REST API.
...
The `public/app/` directory contains a sample frontend. To use your own frontend files:
...
The backend REST API is available at `/api`. All API endpoints documented in the [API Documentation](#api-documentation) section are available for frontend integration:
...
For local development, the PHP built-in server handles both frontend and API:

```bash
php -S localhost:8000 -t public
```

Visit [http://localhost:8000](http://localhost:8000) to see the frontend application.

The frontend is served from the `public/` directory, with the `index.php` file handling API routing via the `.htaccess` rewrite rules.
