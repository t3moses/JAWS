...
2. **Verify email address** (required once):
   ```bash
...
4. **Test email sending** (requires authentication):
   ```bash
   ...
   # Then send notifications (requires admin privileges)
   curl -X POST "http://localhost:8000/api/admin/notifications/Fri%20May%2029" \
     -H "Content-Type: application/json" \
     -H "Authorization: Bearer YOUR_JWT_TOKEN_HERE"
   ```

5. **View sent emails** in LocalStack logs:
   ```bash
   docker-compose -f LocalStack/docker-compose.yml logs -f
   ```

📖 **See also:** [LocalStack/LOCALSTACK_SETUP.md](../LocalStack/LOCALSTACK_SETUP.md) - Complete LocalStack documentation
